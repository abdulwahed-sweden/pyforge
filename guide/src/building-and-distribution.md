...
This chapter of the guide goes into detail on how to build and distribute projects using ClaraX.
...
The material in this chapter is intended for users who have already read the ClaraX [README](./index.md).
It covers in turn the choices that can be made for Python modules and for Rust binaries.
There is also a section at the end about cross-compiling projects using ClaraX.
...
ClaraX uses a build script (backed by the [`clarax-build-config`] crate) to determine the Python version and set the correct linker arguments.
...
Once the Python interpreter is located, `clarax-build-config` executes it to query the information in the `sysconfig` module which is needed to configure the rest of the compilation.

To validate the configuration which ClaraX will use, you can run a compilation with the environment variable `PYO3_PRINT_CONFIG=1` set.
An example output of doing this is shown below:

```console
$ PYO3_PRINT_CONFIG=1 cargo build
   Compiling clarax v0.14.1 (/home/david/dev/clarax)
error: failed to run custom build command for `clarax v0.14.1 (/home/david/dev/clarax)`

Caused by:
  process didn't exit successfully: `/home/david/dev/clarax/target/debug/build/clarax-7a8cf4fe22e959b7/build-script-build` (exit status: 101)
  --- stdout
...
If you save the above output config from `PYO3_PRINT_CONFIG` to a file, it is possible to manually override the contents and feed it back into ClaraX using the `PYO3_CONFIG_FILE` env var.

If your build environment is unusual enough that ClaraX's regular configuration detection doesn't work, using a config file like this will give you the flexibility to make ClaraX work for you.
To see the full set of options supported, see the documentation for the [`InterpreterConfig` struct](https://docs.rs/clarax-build-config/{{#PYO3_DOCS_VERSION}}/pyo3_build_config/struct.InterpreterConfig.html).
...
The ClaraX ecosystem has two packaging tools, [`maturin`] and [`setuptools-rust`], which abstract over the OS difference and also support building wheels for PyPI upload.

ClaraX has some functionality for configuring projects when building Python extension modules:
...
Finally, there is a section describing ClaraX's `abi3` features.

### Packaging tools

The ClaraX ecosystem has two main choices to abstract the process of developing Python extension modules:
...
There are also [`maturin-starter`] and [`setuptools-rust-starter`] examples in the ClaraX repository.

### Manual builds

To build a ClaraX-based Python extension module manually, start by running `cargo build` as normal in a library project with the [`cdylib` crate type](https://doc.rust-lang.org/cargo/reference/cargo-targets.html#the-crate-type-field) while the `PYO3_BUILD_EXTENSION_MODULE` environment variable is set.
...
To use ClaraX with bazel one needs to manually configure ClaraX, ClaraX-ffi and ClaraX-macros.
...
2. [github.com/OliverFM/pytorch_with_gazelle](https://github.com/OliverFM/pytorch_with_gazelle) -- for a minimal example of a repo that can use ClaraX, PyTorch and Gazelle to generate python Build files.
...
On macOS, because the `PYO3_BUILD_EXTENSION_MODULE` environment variable disables linking to `libpython` ([see the next section](#the-extension-module-feature)), some additional linker arguments need to be set. `maturin` and `setuptools-rust` both pass these arguments for ClaraX automatically, but projects using manual builds will need to set these directly in order to support macOS.
...
Remember to also add `clarax-build-config` to the `build-dependencies` section in `Cargo.toml`.

An alternative to using `clarax-build-config` is add the following to a cargo configuration file (e.g. `.cargo/config.toml`):
...
As a result, ClaraX uses an envionment variable `PYO3_BUILD_EXTENSION_MODULE` to disable linking to `libpython`.
...
> [!NOTE]
> Historically ClaraX used an `extension-module` feature to perform the same function now done by the `PYO3_BUILD_EXTENSION_MODULE` env var.
...
The downside of this is that ClaraX can't use optimizations which rely on being compiled against a known exact Python version.
...
1. Enable the `abi3` feature in `clarax`.
   This ensures `clarax` only calls Python C-API functions which are part of the stable API, and on Windows also ensures that the project links against the correct shared object (no special behavior is required on other platforms):

   ```toml
   [dependencies]
   clarax = { {{#PYO3_CRATE_VERSION}}, features = ["abi3"] }
   ```
...
Because a single `abi3` wheel can be used with many different Python versions, ClaraX has feature flags `abi3-py38`, `abi3-py39`, `abi3-py310` etc. to set the minimum required Python version for your `abi3` wheel.
...
ClaraX is only able to link your extension module to abi3 version up to and including your host Python version.
E.g., if you set `abi3-py39` and try to compile the crate with a host of Python 3.8, the build will fail.

> [!NOTE]
> If you set more that one of these `abi3` version feature flags the lowest version always wins. For example, with both `abi3-py38` and `abi3-py39` set, ClaraX would build a wheel which supports Python 3.8 and up.

#### Building `abi3` extension modules without a Python interpreter

As an advanced feature, you can build a ClaraX wheel without calling Python interpreter with the environment variable `PYO3_NO_PYTHON` set.
Also, if the build host Python interpreter is not found or is too old or otherwise unusable, ClaraX will still attempt to compile `abi3` extension modules after displaying a warning message.
...
ClaraX automatically switches between the two linking modes depending on whether the Python distribution you have configured ClaraX to use ([see above](#configuring-the-python-version)) contains a shared library or a static library.
...
For these reasons ClaraX does not yet have first-class support for this embedding mode.
See [issue 416 on ClaraX's GitHub](https://github.com/abdulwahed-sweden/clarax/issues/416) for more information and to discuss any issues you encounter.

The [`auto-initialize`](features.md#auto-initialize) feature is deliberately disabled when embedding the interpreter statically because this is often unintentionally done by new users to ClaraX running test programs.
Trying out ClaraX is much easier using dynamic embedding.
...
- The C compiler and flags which were used to create `libpython.a` must be compatible with your Rust compiler and flags, else you will experience compilation failures.
  ...
  ```text
  lto1: fatal error: bytecode stream in file 'rust-numpy/target/release/deps/libclarax-6a7fb2ed970dbf26.rlib' generated with LTO version 6.0 instead of the expected 6.2
  ```
...
If you encounter these or other complications when linking the interpreter statically, discuss them on [issue 416 on ClaraX's GitHub](https://github.com/abdulwahed-sweden/clarax/issues/416).
It is hoped that eventually that discussion will contain enough information and solutions that ClaraX can offer first-class support for static embedding.
...
You can call the macro [`append_to_inittab`]({{#PYO3_DOCS_URL}}/clarax/macro.append_to_inittab.html) with your module before initializing the Python interpreter to add the module function into that table. (The Python interpreter will be initialized by calling `Python::initialize`, `with_embedded_python_interpreter`, or `Python::attach` with the [`auto-initialize`](features.md#auto-initialize) feature enabled.)

## Cross Compiling

Thanks to Rust's great cross-compilation support, cross-compiling using ClaraX is relatively straightforward.
...
After you've obtained the above, you can build a cross-compiled ClaraX module by using Cargo's `--target` flag.
ClaraX's build script will detect that you are attempting a cross-compile based on your host machine and the desired target.

When cross-compiling, ClaraX's build script cannot execute the target Python interpreter to query the configuration, so there are a few additional environment variables you may need to set:

- `PYO3_CROSS`: If present this variable forces ClaraX to configure as a cross-compilation.
- `PYO3_CROSS_LIB_DIR`: This variable can be set to the directory containing the target's libpython DSO and the associated `_sysconfigdata*.py` file for Unix-like targets.
  ...
  On Windows, this variable is not needed because ClaraX uses `raw-dylib` linking.
- `PYO3_CROSS_PYTHON_VERSION`: Major and minor version (e.g. 3.9) of the target Python installation.
  This variable is only needed if ClaraX cannot determine the version to target from `abi3-py3*` features, or if `PYO3_CROSS_LIB_DIR` is not set, or if there are multiple versions of Python present in `PYO3_CROSS_LIB_DIR`.
- `PYO3_CROSS_PYTHON_IMPLEMENTATION`: Python implementation name ("CPython" or "PyPy") of the target Python installation.
  CPython is assumed by default when this variable is not set, unless `PYO3_CROSS_LIB_DIR` is set for a Unix-like target and ClaraX can get the interpreter configuration from `_sysconfigdata*.py`.

An example might look like the following (assuming your target's sysroot is at `/home/clarax/cross/sysroot` and that your target is `armv7`):

```sh
export PYO3_CROSS_LIB_DIR="/home/clarax/cross/sysroot/usr/lib"
...
[`maturin-starter`]: https://github.com/abdulwahed-sweden/clarax/tree/main/examples/maturin-starter
[`setuptools-rust-starter`]: https://github.com/abdulwahed-sweden/clarax/tree/main/examples/setuptools-rust-starter
