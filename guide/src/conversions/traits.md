...
ClaraX provides some handy traits to convert between Python types and Rust types.
...
There, in order to both be able to operate on mutable references *and* satisfy Rust's rules of non-aliasing mutable references, you have to extract the ClaraX reference wrappers [`PyRef`] and [`PyRefMut`].
...
The `clarax(transparent)` attribute can be used on structs with exactly one field.
...
#             assert!(error.is_instance_of::<clarax::exceptions::PyTypeError>(py));
#         }
#
#         Ok(())
#     })
...
Historically ClaraX has provided a blanket implementation for `#[pyclass]` types that also implement `Clone`, to allow extraction of such types by value.
...
#[pyclass(skip_from_py_object)] // opt-out of the ClaraX FromPyObject blanket
#[derive(Clone)]
struct Number(i32);
...
impl<'py> FromPyObject<'_, 'py> for Number {
    ...
    fn extract(obj: clarax::Borrowed<'_, 'py, clarax::PyAny>) -> Result<Self, Self::Error> {
...
All types in ClaraX implement this trait, as does a `#[pyclass]` which doesn't use `extends`.
...
[`IntoPyObject`]: {{#PYO3_DOCS_URL}}/clarax/conversion/trait.IntoPyObject.html
[`IntoPyObjectExt`]: {{#PYO3_DOCS_URL}}/clarax/conversion/trait.IntoPyObjectExt.html

[`PyRef`]: {{#PYO3_DOCS_URL}}/clarax/pycell/struct.PyRef.html
[`PyRefMut`]: {{#PYO3_DOCS_URL}}/clarax/pycell/struct.PyRefMut.html
[`BoundObject`]: {{#PYO3_DOCS_URL}}/clarax/instance/trait.BoundObject.html
