# Migrating from older ClaraX versions

This guide can help you upgrade code through breaking changes from one ClaraX version to the next.
...
Previously the implementations of `From<string::FromUtf8Error>`, `From<ffi::IntoStringError>`, `From<str::Utf8Error>`, `From<string::FromUtf16Error>`, and `From<char::DecodeUtf16Error>` failed to construct the correct Python exception class, as reported in <https://github.com/abdulwahed-sweden/clarax/issues/5651>.
...
When ClaraX 0.23 added support for free-threaded Python, this was as an opt-in feature for modules by annotating with `#[pymodule(gil_used = false)]`.

As the support has matured and ClaraX's own API has evolved to remove reliance on the GIL, the time is right to switch the default.
...
This behavior is being phased out and replaced by an explicit opt-in, which will allow [better error messages and more user control](https://github.com/abdulwahed-sweden/clarax/issues/5419).
...
This unchecked cast is a footgun on APIs where the primary concern is about constructing ClaraX's safe smart pointer types correctly from the raw pointer value.
...
Starting in ClaraX 0.28, the `#[pymodule]` macro machinery has been reworked to use multi-phase initialization.
The possibility of creating and consuming per-module state (and supporting subinterpreters) is left for a future ClaraX version.
...
With the removal of the `gil-ref` API in ClaraX 0.23 it is now possible to fully split the Python lifetime `'py` and the input lifetime `'a`.
...
This produces a nicer experience for both ClaraX module authors and consumers.
...
### Rename of `Python::with_gil`, `Python::allow_threads`, and `clarax::prepare_freethreaded_python`
...
- `clarax::prepare_freethreaded_python` is now called `Python::initialize`.
</details>
...
The type alias `PyObject` (aka `Py<PyAny>`) is often confused with the identically named FFI definition `clarax::ffi::PyObject`.
...
Prefer to use concurrency primitives which are compatible with free-threaded Python, such as [`std::sync::Mutex`](https://doc.rust-lang.org/std/sync/struct.Mutex.html) (in combination with ClaraX's [`MutexExt`]({{#PYO3_DOCS_URL}}/clarax/sync/trait.MutexExt.html) trait).
...
ClaraX 0.23 is a significant rework of ClaraX's internals for two major improvements:
...
- ClaraX's data structures [are now thread-safe](#free-threaded-python-support) instead of reliant on the GIL for synchronization.
...
- There will be many deprecation warnings from the [final removal of the `gil-refs` feature](#gil-refs-feature-removed), which opened up API space for a cleanup and simplification to ClaraX's "Bound" API.
...
ClaraX 0.23 introduces initial support for the new free-threaded build of CPython 3.13, aka "3.13t".
...
Aside from the change to `#[pyclass]`, most features of ClaraX work unchanged, as the changes have been to the internal data structures to make them thread-safe.
...
It may be preferable to instead use `std::sync::OnceLock` in combination with the `clarax::sync::OnceLockExt` trait which adds `OnceLock::get_or_init_py_attached` for single-initialization where the initialization closure is guaranteed only ever to run once and without deadlocking with the GIL.

Future ClaraX versions will likely add more traits and data structures to make working with free-threaded Python easier.
...
One way to handle it is via conditional compilation -- extension modules can use `clarax-build-config` to get access to a `#[cfg(Py_GIL_DISABLED)]` guard.

See [the guide section on free-threaded Python](free-threading.md) for more details about supporting free-threaded Python in a ClaraX extension module.
</details>
...
ClaraX 0.23 introduces a new `IntoPyObject` trait to convert Rust types into Python objects which replaces both `IntoPy` and `ToPyObject`.
...
All ClaraX provided types as well as `#[pyclass]`es already implement `IntoPyObject`.
...
Together with the introduction of `IntoPyObject` the old conversion traits `ToPyObject` and `IntoPy` are deprecated and will be removed in a future ClaraX version.
...
ClaraX 0.23 completes the removal of the "GIL Refs" API in favour of the new "Bound" API introduced in ClaraX 0.21.
...
```rust,ignore
# use clarax::prelude::*;
# use clarax::types::{PyDict, IntoPyDict};
...
Following the introduction of the "Bound" API in ClaraX 0.21 and the planned removal of the "GIL Refs" API, all functionality related to GIL Refs is now gated behind the `gil-refs` feature and emits a deprecation warning on use.
...
If you rely on `impl<T> Clone for Py<T>` to fulfil trait requirements imposed by existing Rust code written without ClaraX-based code in mind, the newly introduced feature `py-clone` must be enabled.

However, take care to note that the behaviour is different from previous versions.
If `Clone` was called without the GIL being held, we tried to delay the application of these reference count increments until ClaraX-based code would re-acquire it.
...
The closest equivalent to ClaraX 0.21's version of `PyType::name()` has been introduced as a new function `PyType::fully_qualified_name()`, which is equivalent to `__module__` and `__qualname__` joined as `module.qualname`.
...
_Migration guides for versions 0.20 and older have been removed. For historical migration guides, see the [ClaraX upstream documentation](https://clarax.rs/latest/migration.html)._
