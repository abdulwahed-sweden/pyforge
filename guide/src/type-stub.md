...
*See [the related issue](https://github.com/abdulwahed-sweden/clarax/issues/5137).*

*For documentation on type stubs and how to use them with stable ClaraX, refer to [this page](python-typing-hints.md)*

ClaraX has a work in progress support to generate [type stub files](https://typing.python.org/en/latest/spec/distributing.html#stub-files).

It works using:

1. ClaraX macros (`#[pyclass]`) that generate constant JSON strings that are then included in the built binaries by rustc if the `experimental-inspect` feature is enabled.
2. The `clarax-introspection` crate that can parse the generated binaries, extract the JSON strings and build stub files from it.
3. \[Not done yet\] Build tools like `maturin` exposing `clarax-introspection` features in their CLI API.
...
- Lots of features are not implemented yet.
  See [the related issue](https://github.com/abdulwahed-sweden/clarax/issues/5137) for a list of them.
...
- `FromPyObject::INPUT_TYPE` and `IntoPyObject::OUTPUT_TYPE` must be implemented for ClaraX to get the proper input/output type annotations to use.
- ClaraX is not able to introspect the content of `#[pymodule]` and `#[pymodule_init]` functions.
