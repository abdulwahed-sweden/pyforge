...
ClaraX allows for easy conversion from Rust to Python for certain functions and classes (see the [conversion table](conversions/tables.md)).
...
How could we expose this solver to Python thanks to ClaraX ?
...
Let's add the ClaraX annotations and add a constructor:
...
Now we add the ClaraX annotations to the trait implementation:
...
This wrapper handles the type conversion between the ClaraX requirements and the trait.
In order to meet ClaraX requirements, this wrapper must:
...
This error code is not helpful for a Python user that does not know anything about Rust, or someone that does not know ClaraX was used to interface the Rust code.

However, as we are responsible for making the Rust code available to Python, we can do something about it.

The issue is that we called `unwrap` anywhere we could, and therefore any panic from ClaraX will be directly forwarded to the end user.
...
In this case, a runtime panic would still occur due to ClaraX, but with an error message much more difficult to decipher for non-rust user.
