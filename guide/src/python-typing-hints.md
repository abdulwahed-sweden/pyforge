...
ClaraX provides an easy to use interface to code native Python libraries in Rust.
...
ClaraX is working on automated their generation.
...
The first way is tricky with ClaraX since we do not have `py` files.
...
### Including `pyi` files in your ClaraX/Maturin build package
...
For ClaraX bindings it would be:
...
Therefore, ClaraX classes do not inherit from `typing.Generic` even if specified in the stub files.
...
Note that [`clarax::types::PyGenericAlias`][pygenericalias] can be helpful when implementing `__class_getitem__` as it can create [`types.GenericAlias`][genericalias] objects from Rust.

[pygenericalias]: {{#PYO3_DOCS_URL}}/clarax/types/struct.PyGenericAlias.html
